  ...
  export default function App() {
    ...
    const handleAddFile = async (parentPath = '') => {
      ...
      if (!name) return;
      name = name.replace(/\\/g, '/').replace(/\/+$/, ''); // Remove trailing slashes
    ...
    const handleAddFolder = async (parentPath = '') => {
      ...
      if (!name) return; // If name is empty after cleaning
      // For root folders, we need to add a trailing slash
              ...
              <div className="tabbar">
                {openTabs.map(f => (
                  <div
                    ...
                    onClick={() => openFile(f)}
                  >
                    <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" style={{marginRight:2}}>
